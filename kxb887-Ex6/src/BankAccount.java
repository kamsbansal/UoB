...
/**
 * For simplicity we assume here that the balance is represented in Pounds and
 * that only full pounds are used.
 * 
...
public class BankAccount {
	 ...
	 * @param password A string to represent the password. Obviously in a real
	 *                 system we would never store a password in its plain form, but
	 *                 would use a safe way as described on
	...
	/**
	 * Getter method to return the password. As a minimal protection mechanism we
	 ...
	 * 
	 * @param password The password to which the password of the this object is
	 *                 compared.
	...
	/**
	 * Adds the amount - if positive - to the current balance and adds the
	 * transaction to the statement. For non-positive amounts the action does
	 * nothing.
	 * 
	...
	/**
	 * Setter for the amount. Particularly well suited for fraud if made public
	 * rather than private.
	 * 
	...
	/**
	 * withdraws amount from the current balance add transaction to the statement if
	 * there are sufficient funds and the password is correct. Otherwise an
	...
	/**
	 * Transfers an amount of money from the this account to the toAccount assumed
	 * the password is correct and the this account has sufficient money, else an
	 * error message is printed.
	 * 
	...
	/*****************************************************************
	 * Print functions. By keeping these separate from the actual * program it is
	 * easy to adjust them, for instance, for a * different language. *
	 *****************************************************************/
	...
	public void printInsufficientFunds(long amount) {
		System.out.printf("The funds of %s %d for %s are insufficient " + "for this transaction.\n", stringCurrency,
				this.getBalance(), this.getCustomer().getName());
	}
