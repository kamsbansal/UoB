...
public class Main {
        ...
        for(int i=9;i<=17;i++){
            for(int j=0;j<rooms.length;j++){
                ...
                for(int l=0;l<bookings.size();l++){
                    ...
                    if(booking.getDate().equals(date) && booking.getHour()==i && booking.getRoom().equals(rooms[j])){
    ...
    public static void displayRoomChoices(){
        for(int i=0;i<rooms.length;i++){//creating a loop to loop throug the rooms array and displaying each room
            int index=i+1;
